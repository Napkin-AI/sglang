...
class LLaDA2MoeSparseMoeBlock(nn.Module):
    ...
    def _forward_router_experts(self, hidden_states: torch.Tensor, shared_state: Optional[torch.Tensor] = None):
        # router_logits: (num_tokens, n_experts)
        router_logits = self.gate(hidden_states)
        topk_output = self.topk(hidden_states, router_logits)
        if _is_npu:
            return self.experts(hidden_states, topk_output, shared_state)
